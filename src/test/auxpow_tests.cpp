 ...
 */
static void
mineBlock(CBlockHeader& block, bool ok, const Consensus::Params& params, int nBits = -1)
{
    if (nBits == -1)
        nBits = block.nBits;
    ...
    if (ok)
        BOOST_CHECK(CheckProofOfWork(block.GetPoWHash(), nBits, params));
    else
        BOOST_CHECK(!CheckProofOfWork(block.GetPoWHash(), nBits, params));
}
...
BOOST_AUTO_TEST_CASE(auxpow_pow)
{
    ...
    // Use a local copy of consensus params with an easy powLimit for fast scrypt mining.
    // The real powLimit (~uint256(0) >> 20) requires ~1M scrypt hashes per block,
    // which is too slow for unit tests.  ~uint256(0) >> 1 gives ~50% hit rate.
    Consensus::Params params = Params().GetConsensus(371337);
    params.powLimit = ArithToUint256(~arith_uint256(0) >> 1);
    ...
    mineBlock(builder.parentBlock, false, params, block.nBits);
    block.SetAuxpow(new CAuxPow(builder.get()));
    BOOST_CHECK(!CheckAuxPowProofOfWork(block, params));
    mineBlock(builder.parentBlock, true, params, block.nBits);
    block.SetAuxpow(new CAuxPow(builder.get()));
    BOOST_CHECK(CheckAuxPowProofOfWork(block, params));
